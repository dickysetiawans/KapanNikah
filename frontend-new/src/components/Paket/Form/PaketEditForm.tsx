...
export default function PaketEditForm() {
  ...
  // State untuk menampung baris detail fitur (di-set array kosong dulu)
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        // 1. Ambil semua opsi master fitur untuk dropdown
        ...
        if (id) {
          // 2. Ambil data paket utama (Header)
          ...
          // 3. Ambil data detail fitur paket yang sudah tersimpan
          ...
          if (resDetail.data && resDetail.data.length > 0) {
            const mappedDetails = resDetail.data.map((item: any) => ({
              fitur_id: String(item.fitur_id),
              harga_fitur: item.harga_fitur,
            }));
            setDetailFitur(mappedDetails);
          } else {
            // Jika detail kosong dari DB, berikan minimal 1 baris kosong
